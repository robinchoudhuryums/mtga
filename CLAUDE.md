...
- **`deck.py suggest` scopes by CASTABLE COLORS, not identity**, so an off-color
  ...
  **THE LIST IS A WINDOW AND THE RANKING IS THEME FIT, so read a card's ABSENCE as neither
  ...
  (`--limit 0` for all). Why it matters is measured, not asserted: across **948 applied swaps
  that recorded a rank for the card ADDED, the MEDIAN rank is 366** and only **11%** fell
