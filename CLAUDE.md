...
- **`tag_synergies.py` also text-tags MECHANICAL-SYNERGY payoffs the keyword map missed
  ...
  **Residual (by design):** these tags are CORRECT but only surface a fit once the mechanic
  is a *central* theme (the 25%-of-top-theme, floor-2 cutoff). A 2–3-card sub-synergy in a
  deck with a dominant theme (toughness-swap with only Kingpin+Bark; noncombat-damage with
  2 cards under a heavy Wizard theme) stays below "central", so `suggest-homes` won't
  auto-surface it — the tag is right, the centrality gate is the limiter; grade from text.
  A tribal payoff clears easily (a Dino deck runs ~19 Dinosaurs). After editing these
...
still read KEY (anchor 11, the Hawkeye-"KEY"-in-every-Hero-deck fix);
...
2. Analyze a deck — `deck.py check|mana|consistency|tribes|stats|legal|cuts|tier|redundancy|text|verify <id>`, the needs-aware recommenders `deck.py suggest <id> --lands|--ramp|--interaction|--needs`, and roster-wide `deck.py audit` / `deck.py suggest-homes <card>` / `deck.py rotation`. Expect: no traceback; mana is hybrid-aware; consistency reports keepable %/land-drops/cast-on-curve (with the splash / color-hungry fix notes); tribes surfaces type-matters payoffs; legal flags size/copy/format violations; cuts/text print full oracle text; tier shows claimed-vs-floor; redundancy buckets effects by virtual-copy depth and proposes functional copies first, duplicates as fallback; suggest `--lands`/`--needs` surface the STRUCTURAL fills (fixing · acceleration · interaction, with board-scalers flagged) the theme model can't; audit scores every deck TUNE/craft/review/ok; verify diffs a pasted Arena export against the stored deck.
