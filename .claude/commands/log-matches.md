...
**Both line shapes are required.** The JSON carries the result and both seats but NOT
which seat is theirs — the local `userId` appears only in the `Match to <userId>:` header
prefix. A paste of the JSON alone makes every result a coin flip, and the parser refuses
it rather than guessing (it warns and skips). If they only have the JSON, `--me <userId>`
is the escape hatch.
...
put them in a commit. Opponent *deck* (`courseId`) is kept — that is an archetype, not a
person.
...
- **Is the deck mapped?** `courseId` is Arena's identifier (`Avatar_Basic_BlackPanther_MSH`
  for a precon) and there is no way to derive a repo deck id from it. Unmapped rows are
  **kept, not dropped** — see Stage 3.
...
An unmapped row still counts, but it can't be attributed. For each unmapped `courseId` the
report lists, ask the user which repo deck it is, then add the header to that deck file:

```
#: arena: Avatar_Basic_BlackPanther_MSH
```

`parse_matches.py` learns the mapping from those headers, so every future paste resolves
automatically. A `courseId` that is a **precon** (`Avatar_Basic_*`) is usually not one of
the repo's decks at all — say so rather than forcing a mapping.
