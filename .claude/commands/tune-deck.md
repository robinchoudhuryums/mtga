...
5. `python3 scripts/deck.py suggest <id>` — on-color, on-theme pool cards, owned
   ...
   **`--limit 0` ON BOTH HALVES, and the craft half is the one that used to be
   ...
   measured across 973 applied swaps, the MEDIAN rank of the card actually added is
   **368** of ~950 candidates and only **11%** fell inside that window. So a tune
...
auto-filters to the deck's `#: format:`; `--limit 0` for the median-368 reason in
