...
<style>
  ...
  /* The S-8 pass unified the three templates/ pages on the OS colour scheme and stopped
     ...
     this tool in the wrong scheme with no toggle to fix it. Same tokens, same values as
     templates/collection.html, so the two cannot drift apart. */
  ...
  * { box-sizing: border-box; }
  ...
  /* Phone width. The auto-fill grid survives on its own, but the fixed 240px search
     min-width and the 20px padding waste a third of a 390px screen — the same waste
     templates/collection.html documents fixing at S-3, in the sibling that pass skipped.
     Tightened, not restructured. */
  @media (max-width: 620px) {
    header { padding: 12px 14px; }
    .grid { padding: 14px; gap: 12px; grid-template-columns: repeat(auto-fill, minmax(150px, 1fr)); }
...
<header>
  ...
  <div class="controls">
    ...
    <select id="set" aria-label="Filter by set"></select>
...
<script>
  ...
  // esc() on BOTH the attribute and the text. `Set Code` is a free-text CSV column that
  // /api/add stores unvalidated, and this was the one interpolation the BS2-17 escaping
  // pass missed — a set code containing a quote breaks out of value="..." (BS4-43).
