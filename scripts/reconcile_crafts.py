...
front/full name difference); the line's quantity becomes the owned count.
...
def reconcile(export_lines, apply=False):
    pool_by_sc = {}
    pool_by_name = {}
    for r in _read(POOL):
    ...
    added, bumped, mana_added, wish_removed, notfound = [], [], [], [], []
    ...
    for raw in export_lines:
        ...
        elif str(existing.get("Quantity Owned") or "") != str(qty):
            old = existing.get("Quantity Owned") or "0"
            existing["Quantity Owned"] = str(qty)
            bumped.append(f"{front}: {old} -> {qty}")
        ...
        if front.lower() not in mana_names:
            ...
            if src:
                ...
                mana_added.append(front + "  (!! no pool mana row — run build_mana.py)")
...
def main():
    ...
    return reconcile(lines, apply=args.apply)
