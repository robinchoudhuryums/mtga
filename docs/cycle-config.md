...
`check_dfc` grew an **index-alias registry** — seven name-keyed loaders behaviorally
verified to resolve a live DFC's front key, with getattr-at-run-time resolution so a
renamed loader is a hard error — and a **payload pin** on `templates/deck.html`'s
