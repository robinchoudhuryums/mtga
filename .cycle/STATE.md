...
Updated: 2026-09-18 (broad-implement, nonland-source disclosure + pay-life decline)
...
**Docs are the outstanding work across BOTH batches** — `/sync-docs` next: the board-power axis still has no CLAUDE.md rule (every sibling report-only metric has one: G-25, G-60, G-85); G-35 should record the new nonland disclosure; G-41 should record the pay-life decline; each needs a `docs/gotchas.md` long form and a `figure_drift` entry for any figure cited as evidence.
