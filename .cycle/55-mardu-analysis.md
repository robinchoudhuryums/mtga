...
**Status: ORIGINAL PILE COMPLETE — all 131 read, all three decks DRAFTED and landed
(55 Mardu Waves a6f6caa · 55a Spellstorm 2b6774f · 55b Airbender 7d43477); their
durable findings are folded into the deck headers. This doc was deleted at close-out
and RESTORED 2026-08-04 for the ADDENDUM below (§7): 59 additional cards, three new
deck concepts. Do not delete the original analysis — the addendum builds on it.**
A scratchpad, not a source of truth — decks/ are.
...
## 7. ADDENDUM (2026-08-04) — the trimmed remainder: 59 cards, three new concepts

**Source:** the user's FULL pile dump (190 distinct nonbasics). 131 were analyzed above
(the paste's `Eddie Brock` = the doc's `Eddie Brock // Venom`); the delta is **59 new
cards**, not the ~80 estimated. All verdicts here may draw on ORIGINAL-pile cards —
this is an addendum, and the two decks-worth of overlap is the point.

**User's candidate concepts** (grade feasibility, don't assume it):
- **G — Jund-base (RGB)** mana-base deck.
- **J — Jeskai-base (RWU)** mana-base deck.
- **T — red-forward ULTRA-TALL**: boost one/few creatures to lethal in one swing.
Other deck/variant shapes may emerge; equally, a concept may prove infeasible — say so
with counts (G-59/G-61), not vibes.

**Framework carryover:** rules 1–8 and E-01..E-05 above still govern any wave/pulse
reading. New-axis readings add: for T, the deciding counts are (i) evasion/trample
GRANTS (a 40-power body that can be chump-blocked is not a win), (ii) protection
density for the one body, (iii) single-target boost magnitude per mana. For G/J, the
deciding count is what the third color BUYS that Mardu did not already have.

**Batch plan (59 = 19 + 20 + 20, thematic):**
- **A1 (19) — green + white support** (the G concept's new half): Ruby, Daring Tracker
  · Ashroot Animist · Halana and Alena, Partners · Wolverine, Fierce Fighter ·
  Roxanne, Starfall Savant · Voja, Jaws of the Conclave · Samut, the Driving Force ·
  Ghired, Mirror of the Wilds · Annie Joins Up · Quaketusk Boar · Brambleback Brute ·
  Giant Cindermaw · Taii Wakeen, Perfect Shot · Celestial Armor · Due Diligence ·
  Stasis Snare · Twinblade Blessing · Pride of the Road · Charging Strifeknight
- **A2 (20) — the Jeskai block + blue** (the J concept): Sokka, Tenacious Tactician ·
  Jeskai Brushmaster · Flamehold Grappler · Narset, Jeskai Waymaster · Jeskai
  Shrinekeeper · Shiko, Paragon of the Way · Riverwheel Sweep · Monastery Messenger ·
  Jeskai Revelation · Mystic Monastery · Frontier Bivouac · Equilibrium Adept ·
  Effortless Master · Enigma Drake · Crackling Cyclops · Mm'menon, Uthros Exile ·
  Spectacular Skywhale · Speedball, New Warrior · Zidane, Tantalus Thief · Sami,
  Wildcat Captain
- **A3 (20) — red tall / equipment / burst** (the T concept): Red Hulk · Molten Man,
  Inferno Incarnate · Gilgamesh, Master-at-Arms · Twin Blades · Samurai's Katana ·
  Red Mage's Rapier (re-read for T) · Haste Magic · Ancestors' Aid · Self-Destruct ·
  The Last Agni Kai · Reckless Ransacking · Raging Goblinoids · Red Tiger Mechan ·
  Speed, Young Avenger · Team Tactics · Mai, Jaded Edge · Rocketeer Boostbuggy ·
  War Machine, Legacy of Iron · Iron Man, Master of Machines · Spider Manifestation ·
  Scarlet Spider, Ben Reilly

*(A3 lists 21 names because Red Mage's Rapier was graded in the original pile for the
Mardu axes and gets a RE-read for T only.)*

### Addendum Batch A1 (19 — green + white support)

**The batch's headline finding, stated early because it shapes concept G:** the combined
pile contains **ZERO green-black cards**. Every green card here is R/G (Ruby, Ashroot,
Halana, Wolverine, Roxanne) or NAYA — W/R/G (Voja, Samut, Ghired, Annie), and a Naya
identity CANNOT sit in an RGB deck. An "RGB mana base" would be a Gruul shell + a black
splash with no G↔B synergy card anywhere in the pile; meanwhile the same green cards
actively point at GRUUL-TALL (Ashroot/Halana are the two best tall enablers read so
far) or a NAYA legendary shell (Annie doubles legendary triggers). Verdict deferred to
close-out per protocol, but the count is the count (G-59/G-61).

| Card | Grade | Axis | Note |
|---|---|---|---|
| Ashroot Animist | ★★★ | T | attack: ANOTHER creature +X/+X AND trample, X = its power — the tall deck's core enabler shape (pump + the evasion grant in one card) |
| Halana and Alena, Partners | ★★★ | T | each combat: X PERMANENT counters (X = their power) + haste on another creature — compounds every turn; owned ×1 |
| Celestial Armor | ★★ | T | flash Equipment: ETB attach + hexproof/indestructible THAT turn (a protection trick), then +2/+0 flying — protection AND evasion for the one body; owned, already in 3 decks |
| Twinblade Blessing | ★★ | T | flash double-strike aura — literally doubles the boosted swing; the T-axis multiplier |
| Wolverine, Fierce Fighter | ★★ | T/G | ETB fight (removal on a body) + damage never accumulates on him — a resilient bearer; owned ×1 |
| Ruby, Daring Tracker | ★★ | G-infra | RG dork with haste; pump gated on power-4+ (auto-on in a tall deck); the Gruul Mardu-Devotee |
| Samut, the Driving Force | ★★ | Naya | team +X/+0 and {X}-off noncreature spells, X = speed — real but NAYA identity, and speed wants early aggro |
| Ghired, Mirror of the Wilds | ★★ | Naya | nontoken creatures tap to COPY a token that entered this turn — a copied Mobilize token does NOT carry the sac trigger (it copies the token, not the delayed trigger), so Ghired makes waves PERMANENT... in a deck 55 cannot cast (G identity). Recorded as the pile's biggest off-identity tease |
| Annie Joins Up | ★★ | Naya | doubles LEGENDARY creatures' triggered abilities + ETB 5 damage — the Naya shell's Windcrag; only lands there |
| Roxanne, Starfall Savant | ★ | G | Meteorite tokens = burn + ramp that DOUBLES when tapped; 5 MV and the payoff wants artifact-token support the pile lacks (Crime Novelist's count moves +1 if she lands) |
| Taii Wakeen, Perfect Shot | ★★ | B-axis | NOT a tall card — the original pile's PINGER axis (B) gains a payoff: draw on exact-toughness noncombat kills + {X},{T} amplifier. RW, castable in 55/55a; with Stadium Headliner, Judith, Cori, Sharpshooter this is now a 5-member Boros-pinger cluster worth watching |
| Charging Strifeknight | ★ | 55a bench | {T}, discard: draw on a hasty body — a DISCARD OUTLET, i.e. 55a's missing third outlet for UGG/mayhem; rule-6 tap cost is fine there (no width engine) |
| Stasis Snare | ★ | J/any-W | flash exile removal; generic glue for whichever white deck wants it; owned, in 5 decks |
| Pride of the Road | ◇ | T | Max-speed double-strike grant each combat — gated on reaching speed 4, so it pays off only in an already-aggressive shell |
| Brambleback Brute | △ | T | two sorcery-speed "can't block" uses on a 2-power body — removes ONE blocker at a time; tall wants trample/unblockable grants, not this |
| Quaketusk Boar | △ | — | reach/trample/haste statline, no text — filler in every concept here |
| Giant Cindermaw | △ | meta | "players can't gain life" on a cheap trampler — a sideboard-shaped stax card; no axis wants the body; owned (deck 28) |
| Due Diligence | △ | — | +2/+2 vigilance aura with a minor ETB — outclassed by every T aura already read |
| Voja, Jaws of the Conclave | ✗ | — | G-61 by the numbers: counters scale on ELVES you control (combined pile count: 0) and draw on WOLVES (count: himself) — in these piles he is a 5/5 ward-3 that draws 1 per attack for 5 mana in a THIRD color; a Voja deck needs the Elf half we do not have |

**Cross-links recorded:** Taii Wakeen → original axis B (pinger cluster now 5 real
members — a possible Boros-pinger variant if T/G/J stall); Charging Strifeknight →
