# Cycle history — narrative record

Completed-cycle narrative, split out of `.cycle/STATE.md` on 2026-09-08.

STATE.md is ROLLING: it carries the CURRENT cycle only, in exactly the seven
sections the workflow template defines, because `/cycle-status` and
`/cycle-resume` read the FIRST match of each heading. Left unchecked it
accumulates one section per session until the substrate a new session loads is
buried — it had reached **45 sections / 2,913 lines with 66 "Where I left off"
headings**, against the 24-section/347-line shape the template repo diagnosed as
a defect in its own copy (v1.29.0 F15). Everything below is that history,
verbatim and newest-first; nothing was dropped in the split.

**This file is a record, not instructions.** For what to DO next read
`.cycle/NEXT-SESSION.md`; for the current cycle's machine-readable state read
`.cycle/STATE.md`. Per-run implementation summaries live in `.cycle/blocks/`.
Read a section here to find out why something was decided, or before
re-proposing a fix that was already declined.
...
tapped unless you control a basic land", where any basic satisfies it, and the TYPE-NAMED
cycle, "unless you control a Plains or an Island". Both returned kind `check`, and the
only thing any consumer knew was the deck's TOTAL basic count against
`_CHECKLAND_BASIC_FLOOR = 12`. A deck's basic count says nothing about whether it runs the
type the gate NAMES, so a land that can never be untapped in a deck took the untapped
premium there anyway.

**Found from the other end.** Nobody audited the predicate. A manabase pass on deck 2
(mono-red, 23 Mountain) had `suggest 2 --lands` rank Cori Mountain Monastery at #2, scored
7.2, labelled `·check` — tied with Fire Nation Palace, which genuinely is untapped there.
Its gate is a Plains or an Island. Reading a tool's output as a user is what surfaced a
bug 54 decks wide.

**Measured before the fix**, over on-colour candidates only: **81 (deck, land) pairs
across 54 of 114 decks**. Verified by hand — deck 11 (Swamp only) offered Great Arashin
City, gated on a Forest or a Plains; deck 50 (Forest only) offered Kishla Village, gated
on an Island or a Swamp.

**The fix went in the predicate, not the callers**, because G-35's whole point is that
there is ONE of it. `tapland_check_types` reads the gate's basics off NAMED CAPTURE GROUPS
added to the existing pattern — a second regex for "which types" is the parallel-source
shape this project keeps paying for. `tapland_kind(text, basic_types=None)` returns
`unconditional` for an unmeetable named gate, which is the literal truth and keeps it out
of `TAPLAND_CONDITIONAL_KINDS`. Omitting the argument reproduces every pre-fix answer, so
`wishlist --rank` (deckless) is untouched. Three callers opted in: the premium, the
`·check` RIDER (which was making the same wrong claim to the reader), and
`tapland_profile`'s tempo line.

**Roster diff**, measured by running `suggest_lands` over all 112 decks twice with the
predicate monkeypatched back: **79 scores moved, 91 rider labels corrected, 63 decks
changed a row, 6 decks' #1 pick changed** — every one off a land it can never untap.

**The second finding is the G-53 shape one layer up.** `/tune-deck` gated `suggest
--lands` on "if the scorecard says the deficit is … mana", which is right for `--ramp`
and `--interaction` and became wrong for `--lands` on 2026-09-20, when G-37 made a land
already in the deck a pick: the recommender's commonest output became advice for a
manabase that is NOT deficient. Deck 2 is the worked case — R 24, every card casting on
curve at >= 90%, no scorecard would have opened the list, and a free second Fire Nation
Palace was sitting in it. Step 5c runs it every time now, and says to read the TEMPO line
rather than the colour counts, because an unconditional tapland swapped for an untapped
dual of the same colours moves no probability figure at all.

**DECLINED, so it is not re-proposed:** feeding the tempo delta into a score. G-35 keeps
`tapland_profile` report-only for the G-25/G-60 reason, and nothing here changed that.

Full detail: `.cycle/blocks/2026-09-checkland-types-and-tune-deck-land-gate-broad-implement.md`,
plus two new dated sections in `docs/gotchas.md` under G-35 and G-37.
...
(Moved verbatim from STATE.md's "Where I left off" on 2026-09-23, when that rolling
section was replaced. STATE.md is the CURRENT cycle only; narrative belongs here, and
this entry had never been carried across.)
